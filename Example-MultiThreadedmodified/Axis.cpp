...
/* void Axis::SetThreeMoves(int32_t move1Counts, int32_t move2Counts, int32_t move3Counts) {
    ...
    m_move2.type = MG_MOVE_STYLE_NORM;
    m_move2.value = move1Counts + move2Counts;

    m_move3.type = MG_MOVE_STYLE_NORM;
    m_move3.value = move1Counts + move2Counts + move3Counts;
...
/* void Axis::ExecuteThreeMoves() {
    printf("[Hip motor 1] Executing Move1 to %d...\n", m_move1.value);
    m_node->Motion.MovePosnStart(m_move1.value, true, false, false, false);
    while (!m_node->Motion.MoveIsDone()) {}

    printf("[Hip motor 1] Executing Move2 to %d...\n", m_move2.value);
    m_node->Motion.MovePosnStart(m_move2.value, true, false, false, false);
    while (!m_node->Motion.MoveIsDone()) {}

    printf("[Hip motor 1] Executing Move3 to %d...\n", m_move3.value);
    m_node->Motion.MovePosnStart(m_move3.value, true, false, false, false);
    while (!m_node->Motion.MoveIsDone()) {}
}
 */
